...
const ProjectCard: React.FC<{ 
  ...
  const isCurrentlyActive = (activeId === project.id || isHero) && !hasVideoError;
  ...
  useEffect(() => {
    if (isCurrentlyActive && videoRef.current && !hasVideoError) {
      const playPromise = videoRef.current.play();
      if (playPromise !== undefined) {
        playPromise.catch((e) => {
          // If unmuted autoplay is blocked by browser policy, mute and try again
          if (videoRef.current && !hasVideoError) {
            videoRef.current.muted = true;
            setIsMuted(true);
            videoRef.current.play().catch((err) => {
              // Silently handle autoplay restriction without breaking the UI
              console.warn("Autoplay deferred until user interaction:", err?.message || err);
            });
          }
        });
      }
  ...
  }, [isCurrentlyActive, project.id, hasVideoError]);
...
const Work: React.FC = () => {
  ...
  const handleAddProject = async (e: React.FormEvent) => {
    ...
    setErrorMessage(null);
    try {
      const vUrl = await uploadToStorage(newProject.videoFile, 'videos');
  ...
  const handleSetHero = async (id: string, category: string) => {
    try {
      setIsLoading(true);
  ...
  const renderShowcase = (category: string) => {
    ...
    // 1. Explicit hero (is_hero = true) only — no automatic fallback hero!
    const hero = categoryProjects.find(p => p.isHero);
    
    // 2. Others: All non-hero projects, sorted by sortOrder DESC (newest first)
